...
RunBGLR<-function(YearEffects=TRUE,nIter=50000,burnIn=40000,
  ...
  fm=BGLR(y=y,
          ...
          nIter=nIter,
          burnIn=burnIn,
          saveAt=paste0(WD,Outputfiledir,"Model_withFMLoc"),
          verbose=TRUE)
  yHat<-fm$yHat
...
predict<-function(testing=testing,yBLUE=yBLUE,Y=Y,fmfiledir){
  ...
  write.csv(predict,paste0(WD,fmfiledir,"TP_predict_PP_withFMloc.csv"))
...
#### 4. 
yHatVarMean_withloc<-function(filedir,vB=3,vGCA1=4,vGCA2=5,vSCA=6,filename="Model_withFMLoc"){
  ...
  #### Save the plot!!!!!
  pdf(paste0(WD,filedir,"varE_varGCA1_2_SCA_withLoc.pdf"))
  par(mfrow=c(2,2))
  ...
  write.csv(varMean,paste0(WD,filedir,"varMean_withLoc.csv"))
