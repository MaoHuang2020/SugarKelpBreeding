...
source(paste0(WD,"OneTime1920/code/","BGLR_functions_withFMLoc.R")) # terminal
...
for (i in 1:reps){
  ...
  tmp<-NULL
   for (fold in folds){
     ...
     yPred<-fm$ETA[[6]]$u+fm$ETA[[5]]$u+fm$ETA[[4]]$u  #SCA+GCA2+GCA1
  ...
  cor[i,]<-cor(tmp[tmp$popChk=="ES",]$yBLUE,tmp[tmp$popChk=="ES",]$yPred,use="complete")
...
write.csv(cor,"10foldCV_cor2_using_250_ESplots_withFMLoc.csv")

cor2<-matrix(nrow=reps,ncol=1)
for (i in 1:reps){
  savepath<-paste0("OneTime1920/Alldata_CV_output/withFMLocRep",i,"/")
  Ypred<-read.csv(paste0(WD,savepath,"predictions_rep",i,".csv"),sep=",",header=TRUE)
  cor2[i,]<-cor(Ypred[Ypred$popChk=="ES",]$yBLUE,Ypred[Ypred$popChk=="ES",]$yPred,use="complete")
}
  colMeans(cor2) #0.1827861     #noLOC 0.2699811
write.csv(cor2,"10foldCV_cor2_using_250ES_plots2_WithFMLoc.csv")


### All data GCA1, GCA2, SCA components
#noLoc
reps<-20
varfm_noloc<-NULL #showed 5 varcomp
varfm_wloc<-NULL
for (i in 1:reps){
  varcomp_noloc<-yHatVarMean_noloc(filedir = paste0("OneTime1920/Alldata_output/noLocRep",i,"/"),vB=2,vGCA1=3,vGCA2=4,vSCA=5,filename="Model_noFMLoc")$varMean[2,]
  varfm_noloc<-rbind(varfm_noloc,varcomp_noloc)
  
  varcomp_withloc<-yHatVarMean_withloc(filedir = paste0("OneTime1920/Alldata_output/withFMLocRep",i,"/"),vB=3,vGCA1=4,vGCA2=5,vSCA=6,filename="Model_withFMLoc")$varMean[2,]
  varfm_wloc<-rbind(varfm_wloc,varcomp_withloc)
}

colMeans(varfm_noloc)
#noloc
#0.046349921 0.010714648 0.005839523 0.001932166 0.003613894

colMeans(varfm_wloc)
#withloc
#0.053026136 0.011174054 0.002943809 0.001698484 0.003165850
