...
CrossBLUE<-Both_dBLUPs ### Subset the Yr
...
IDs<-as.character(unique(rownames(CrossBLUE)))  ### RM checks??
...
### Load Sample file
load(paste0(datafdr,"sampleCV_250Indiv_0303_2021.Rdata"))  
# 250 individuals in the sampleCV. But CrossBLUE only 244 lines
sampleCV<-sampleCV[-c(1:6),]  ##
folds<-1:10 
reps<-20 # !!!
...
for (j in 1:length(traits)){
  ...
  for (i in 1:reps){
    setwd(paste0(WD,"Alldata_CV_output/"))
    dir.create(paste0(Coltrait,"_Yr",Yr,"_ydrBLUPsnoLocRep",i))
    savepath<-paste0("Alldata_CV_output/",Coltrait,"_Yr",Yr,"_ydrBLUPsnoLocRep",i,"/")  # the path within WD!
...
write.csv(cor_std,paste0(paste0(WD,"Alldata_CV_output/","cor_CV_Yr",Yr,"_ydrBLUPs_data_",length(traits),"Traits_Mean_05272021.csv")))
write.csv(cor,paste0(WD,"Alldata_CV_output/","cor_CV_Yr",Yr,"_ydrBLUPs_data_",length(traits),"Traits_05272021.csv"))
