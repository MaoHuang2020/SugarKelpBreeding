...
CalGCA<-function(Y=Y,mm.file=mm.file,colIDy=3,colNam="P1",savefiledir="Onetime1920/Yr19_20/GP1/"){
  ...
  save(GCA,file=paste0(WD,savefiledir,"G.rda"))  ###!!! GP1/
  EVD<-eigen(GCA)
  rownames(EVD$vectors)<-rownames(GCA)
  save(EVD,file=paste0(WD,savefiledir,"EVD.rda")) ###!!! GP1/
...
CalSCA<-function(G1.file=G1.file,G2.file=G2.file,savefileDir="Onetime1920/Yr19_20/GP1P2/"){
  ...
  GI <- G1*G2 
  EVD <- eigen(GI)
  save(GI,file=paste0(WD,savefileDir,"G.rda"))
  save(EVD,file=paste0(WD,savefileDir,"EVD.rda"))
}
...
RunBGLR<-function(YearEffects=TRUE,nIter=50000,burnIn=40000,
  ...
  fm=BGLR(y=y,
          ...
          nIter=nIter,
          burnIn=burnIn,
          saveAt=paste0(WD,Outputfiledir,"Model_noFMLoc"),
          verbose=TRUE)
  yHat<-fm$yHat
    save(fm,file=paste0(WD,Outputfiledir,"fm.rda"))
  y<-fm$y
  return(list(fm=fm,yHat=fm$yHat))
}

#### 3.2 noFMLoc
predict<-function(testing=testing,gid=gid,yBLUE=yBLUE,Y=Y,fmfiledir){
  load(paste0(WD,fmfiledir,"fm.rda")) 
  fm<-fm
  ...
  predict$popChk<-vlookup(predict$Crosses,dict=Y,lookup_column = "Crosses",result_column = "popChk")
  predict$Year<-Y[predict$testing,]$Year  # some Crosses are dup between years
    write.csv(predict,paste0(WD,fmfiledir,"TP_predict_PP_noFMLoc.csv"))
...
#### 4. 
yHatVarMean<-function(filedir){
  load(paste0(WD,filedir,"fm.rda")) 
  fm<-fm
  varfm<-c(fm$varE,fm$ETA[[3]]$varB,fm$ETA[[4]]$varU,fm$ETA[[5]]$varU,fm$ETA[[6]]$varU)
  
  varE<-scan(paste0(WD,filedir,"OneTime_With_CheckvarE.dat"))
  varB<-scan(paste0(WD,filedir,"OneTime_With_CheckETA_3_varB.dat"))
  varU1<-scan(paste0(WD,filedir,"OneTime_With_CheckETA_4_varU.dat"))
  varU2<-scan(paste0(WD,filedir,"OneTime_With_CheckETA_5_varU.dat"))
  varU3<-scan(paste0(WD,filedir,"OneTime_With_CheckETA_6_varU.dat"))
  
  # plot(varU3,type='o',col=2,cex=.5)
  #### Save the plot!!!!!
  pdf(paste0(WD,filedir,"varE_varGCA1_2_SCA.pdf"))
  par(mfrow=c(2,2))
  ...
  varMean<-rbind(varMean,varfm)
    write.csv(varMean,paste0(WD,filedir,"varMean.csv"))
  return(varMean)
}
